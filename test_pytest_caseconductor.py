...
def test_summary_is_not_output_when_no_id_in_marker(testdir):
    ...
    assert result.ret == 0
    py.test.raises(Exception, result.stdout.fnmatch_lines, ['*Case Conductor*'])
...
def test_summary_when_single_id_in_marker(testdir):
    ...
    result = testdir.runpytest('--verbose')
    assert result.ret == 0
    result.stdout.fnmatch_lines(['*Case Conductor*',
                                 '1234*'])

def test_summary_when_multiple_ids_in_marker(testdir):
    testdir.makepyfile("""
        import pytest
        @pytest.mark.cc([1234, 1235])
        def test_whatever():
            pass
        """)
    result = testdir.runpytest('--verbose')
